...
```
### v2.6.1 (2026-04-29)
- COLDLOCK тепер повністю вимикає витяжку (раніше залишав на CH1/CH3)
- Periodic screen recovery не ламає нічний режим дисплея (drawStaticUI замінено на resetDisplayCache)
- Blynk стан (connected/offline) коректно відображається при DHT помилці
- Після reconnect Blynk синхронізує всі віртуальні піни (V3, V7, V8, V9, V14)
- DHT не читається частіше ніж раз на 2с при оновленні вологості через V6
- День/ніч режим визначається коректно при старті (фоторезистор читається до першого циклу)
- Сірі індикатори холоду/обігріву замінені на чорні (невидимі коли неактивні)
- checkCycleTransition логує стан при Fan OFF
